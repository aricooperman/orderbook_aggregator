...
impl ExchangeWebSocketApi for BinanceWebSocketApi {
        ...
        match serde_json::from_str::<Value>(msg) {
            Ok(value) => {
                match value.as_object() {
                    None => Err(KeyrockError::WebSocketReceiveError(format!(
                    ...
                    Some(map) => {
                        ...
                        if map.contains_key("result") {
                            ...
                            if id.is_null() || !id.is_u64() {
                                return Err(KeyrockError::WebSocketReceiveError(format!(
    ...
    fn check_orderbook_subscription_params(&self, _symbol: &str, depth: usize) -> Result<()> {
        if depth != 5 && depth != 10 && depth != 20 {
            return Err(KeyrockError::BadArgument(format!(
        ...
        if let Some(sub) = Self::get_primary_orderbook_subscription(&self.orderbook_subscriptions) {
            Err(KeyrockError::SubscriptionError(format!("Binance API v2 only supports one orderbook subscription simultaneously and is already subscribed to {}", sub)))
        } else {
            Ok(())
        }
    }
